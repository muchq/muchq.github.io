  ...
  <>
    <div className={styles.section}>
      ...
      <p className={own.note}>
        What happened inside games.muchq.com, which the access log cannot see: a
        session opens one socket and every room, world, table and message rides
        it. {n(hub.rooms.created)} rooms made, {n(hub.rooms.closed)} closed,{" "}
        ...
        <table className={styles.containerTable} data-testid="hub-days">
          ...
          <tbody>
            ...
            {hub.days.length === 0 && (
              <tr>
                <td colSpan={4}>
                  {hubEvents ? "Nobody has opened a room yet." : UNAVAILABLE}
                </td>
              </tr>
            )}
          </tbody>
    ...
    <div className={styles.sectionGrid}>
      <div className={styles.section}>
        ...
        <p className={own.note}>
          A table is abandoned when too few seats are left to go on, which is
          most of the ways a game ends when nobody is watching.
        </p>
        <div className={styles.tableScroll}>
          <table className={styles.containerTable} data-testid="hub-variants">
            ...
            <tbody>
              ...
              {hub.variants.length === 0 && (
                <tr>
                  <td colSpan={4}>
                    {hubEvents ? "No tables dealt yet." : UNAVAILABLE}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className={styles.section}>
        ...
        <p className={own.note}>
          Seats a table was dealt to, counted while it was still whole. How many
          were left at the end is a different number and not this one.
        </p>
        <div className={styles.tableScroll}>
          <table className={styles.containerTable} data-testid="hub-sizes">
            ...
            <tbody>
              {hub.sizes.map((size) => (
                <tr key={size.players}>
                  <td>
                    {size.players === OVER_CAP
                      ? "More than a table seats"
                      : n(size.players)}
                  </td>
                  <td>{n(size.dealt)}</td>
                </tr>
          ...
          <table className={styles.containerTable} data-testid="hub-surfaces">
            ...
            <tbody>
              ...
              {hub.surfaces.length === 0 && (
                <tr>
                  <td colSpan={3}>
                    {hubEvents ? "No rooms yet." : UNAVAILABLE}
                  </td>
                </tr>
              )}
            </tbody>
